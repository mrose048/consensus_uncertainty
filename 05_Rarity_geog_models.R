...
p0 <- ggplot(data = exp_sd, aes(x = prev, y = sd_HSC)) +
  geom_point() +
...
p1 <-
  ggplot(data = exp_sd, aes(x = area100, y = log10(sd_HSC))) +
  geom_point() +
  geom_smooth(method = 'glm') +
...
p2 <-
  ggplot(data = exp_sd, aes(x = niche_breadth, y = log10(sd_HSC))) +
  geom_point() +
  geom_smooth(method = 'glm') +
...
p3 <-
  ggplot(data = exp_sd, aes(x = range_elev, y = log10(sd_HSC))) +
  geom_point() +
  geom_smooth(method = 'glm') +
...
p4 <-
  ggplot(data = exp_sd, aes(x = range_topo, y = log10(sd_HSC))) +
  geom_point() +
  geom_smooth(method = 'glm') +
...
ggsave(
  ...
  dpi = 500,
  unit = 'cm',
  file = 'Dissertation/003_Ensemble_modeling/figures/rarity_geog_and_uncertainty.png'
)
...
mod_form <-
  formula(
    log(sd_HSC) ~ scale(prev) + scale(area100) + scale(niche_breadth) + scale(mean_elev) + scale(mean_topo) + scale(range_elev) + scale(range_topo)
  )
...
p0 <- all_sd %>% filter(dispersal == 'nd_change_prop') %>%
  split(.$source) %>%
  purrr::map(
    ~ glm(
      sd_HSC ~ scale(prev) + scale(area100) + scale(niche_breadth) + scale(mean_elev) + scale(range_elev) + scale(mean_topo) + scale(range_topo),
      data = .x
    )
  ) %>%
  dwplot(whisker_args = c(size = 1.5),
  ...
  scale_colour_viridis_d(
    ...
    name = "Source of ucertainty",
    breaks = c("model", "gcm", "rcp"),
    labels = c("Consensus method", "GCM", "RCP")
  ) +
  theme(plot.title = element_text(face = "bold", hjust = 0.5)) +
  ggtitle('Null Dispersal')



p1 <- all_sd %>% filter(dispersal == 'fd_change_prop') %>%
  split(.$source) %>%
  purrr::map(
    ~ glm(
      log(sd_HSC) ~ scale(prev) + scale(area100) + scale(niche_breadth) + scale(mean_elev) + scale(range_elev) + scale(mean_topo) + scale(range_topo),
      data = .x
    )
  ) %>%
  dwplot(whisker_args = c(size = 1.5),
...
ggsave(
  ...
  dpi = 500,
  unit = 'cm',
  file = '/My Drive/Dissertation/003_Ensemble_modeling/figures/rarity_geog_and_rcp_based_uncertainty.png'
)
...
ggsave(
  ...
  dpi = 500,
  unit = 'cm',
  file = '/Volumes/GoogleDrive/My Drive/Dissertation/003_Ensemble_modeling/figures/coefficient plots_v2.png'
)
